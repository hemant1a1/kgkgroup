...
export default function SourcingWorld() {
  ...
  const handleMouseMove = (e) => {
    ...
    const rotateY = xRel * 25;       // Horizontal = Y-axis tilt
    const rotateX = -yRel * 25;      // Vertical = X-axis tilt
    const rotateZ = xRel * 10;       // Horizontal = clockwise/counter spin
  ...
  visible: {
    transition: {
      staggerChildren: 0.15, // controls delay between each li
    },
  },
};
            ...
            <div>
              ...
              >
                <motion.ul className="space-y-1 list-disc list-inside" variants={listVariants}>
                  {['Brazil', 'Colombia', 'Madagascar', 'Mozambique'].map((country) => (
                ...
                <motion.ul className="space-y-1 list-disc list-inside" variants={listVariants}>
                  {['Sri Lanka', 'Tanzania', 'Zambia'].map((country) => (
                    <motion.li key={country} variants={itemVariants}>
                      {country}
                    </motion.li>
                  ))}
